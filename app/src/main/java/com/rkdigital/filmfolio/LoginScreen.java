...
public class LoginScreen extends AppCompatActivity implements SharedPreferenceObserver.OnPreferenceChangeListener {
    ...
    private void signInWithGoogle() {
        ...
        GetSignInWithGoogleOption googleIdOption = new GetSignInWithGoogleOption.Builder(getString(R.string.default_web_client_id))
                .build();
        ...
        CredentialManagerCallback<GetCredentialResponse, GetCredentialException> callback = new CredentialManagerCallback<GetCredentialResponse, GetCredentialException>() {
            ...
            public void onError(GetCredentialException exception) {
                ...
                runOnUiThread(() -> Toast.makeText(LoginScreen.this, "Google Sign-In Failed: " + exception.getMessage(), Toast.LENGTH_SHORT).show());
            }
        };
    ...
    private void handleSignIn(Credential credential) {
        if (credential instanceof CustomCredential &&
            ...
            try {
                ...
                Log.e("GoogleSignIn", "Failed to parse Google ID Token Credential", e);
                runOnUiThread(() -> Toast.makeText(this, "Parsing error", Toast.LENGTH_SHORT).show());
            }
        } else if (credential instanceof GoogleIdTokenCredential) {
             firebaseAuthWithGoogle((GoogleIdTokenCredential) credential);
        } else {
            Log.e("GoogleSignIn", "Unexpected credential type: " + credential.getType());
            runOnUiThread(() -> Toast.makeText(this, "Unexpected credential type", Toast.LENGTH_SHORT).show());
        }
    }

    private void firebaseAuthWithGoogle(GoogleIdTokenCredential googleCredential) {
            ...
            mAuth.signInWithCredential(firebaseCredential)
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            ...
                            runOnUiThread(() -> Toast.makeText(getApplicationContext(), "Firebase Auth failed", Toast.LENGTH_SHORT).show());
                        }
                    });
        } else {
            runOnUiThread(() -> Toast.makeText(getApplicationContext(), "Failed to retrieve ID Token", Toast.LENGTH_SHORT).show());
        }
    }
